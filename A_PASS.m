...
s.isscore_check = uicontrol('Parent',s.hf,'Style','checkbox','String','Sleep stage scoring','Units','normalized','FontSize',12,...
    'Position',[0.08 0.65 0.25 0.03],'FontWeight','bold','Value',1);
s.autoscore_check = uicontrol('parent',s.hf,'style','checkbox','string','Automatic scoring','FontSize',12,'units','normalized',...
...
set(s.isscore_check,'callback',{@isscore_check_callback,s});
set(s.autoscore_check,'callback',{@autoscore_check_callback,s});
...
set(s.metric_check,'callback',{@metric_check_callback,s});
...
function eegpre_check_callback(hObject,eventdata,s)
global A_Cfg
A_Cfg.eegpre = get(hObject,'value');
...
function isscore_check_callback(hObject,eventdata,s)
global A_Cfg
v=get(hObject,'value');
A_Cfg.score = (get(hObject,'value'));
if v==0
    set(s.autoscore_check,'enable','off');
    set(s.manualscore_check,'enable','off');
    A_Cfg.autoscore=0;
    A_Cfg.manuscore=0;
else
    
    set(s.autoscore_check,'enable','on');
    set(s.manualscore_check,'enable','on');
    A_Cfg.autoscore=1;
end
A_Cfg.isscore=v;

function autoscore_check_callback(hObject,eventdata,s)
global A_Cfg
A_Cfg.autoscore = get(hObject,'value');
...
v=get(hObject,'value');
if v==0
    set(s.dicom2nii_check,'enable','off')
    set(s.ep_length_edit,'enable','off');
    set(s.metric_check,'enable','off');
    set(s.ALFF_check,'enable','off');
    set(s.ReHo_check,'enable','off');
    set(s.DC_check,'enable','off');
    set(s.VMHC_check,'enable','off');
    ...
    set(s.ep_length_edit,'enable','on');
    set(s.ALFF_check,'enable','on');
    set(s.ReHo_check,'enable','on');
    set(s.DC_check,'enable','on');
    set(s.VMHC_check,'enable','on');
    set(s.FC_check,'enable','on');
    set(s.ROIadd_button,'enable','on');
...
function metric_check_callback(hObject,eventdata,s);
global A_Cfg
v=get(hObject,'value');
if v==0
    set(s.ALFF_check,'enable','off');
...
A_Cfg.metrics=(get(hObject,'value'));
...
if get(hObject,'value')==0
    set(s.voxelp_pop,'enable','off');
    set(s.clusterp_pop,'enable','off');
    
else
    set(s.voxelp_pop,'enable','on');
    set(s.clusterp_pop,'enable','off');
end
...
%     end
%   
    set(s.eegpre_check,'value',Cfg.eegpre);
    %set(s.MRc_check,'value',Cfg.MRc);
    %set(s.BCGc_check,'value',Cfg.BCGc);
    set(s.isscore_check,'value',Cfg.isscore);
    if Cfg.isscore==0
        set(s.autoscore_check,'enable','off');
        set(s.manualscore_check,'enable','off');
    else
    ...
    set(s.autoscore_check,'value',Cfg.autoscore);
...
%     set(s.conda_edit,'string',Cfg.conda_path);
    set(s.metric_check,'value',Cfg.metrics);
