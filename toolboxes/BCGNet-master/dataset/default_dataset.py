...
class Dataset:
    ...
    def save_databv(self, p_output, f_output, overwrite=False, idx_fold=None, **kwargs):
        """
        Save the processed dataset in BrainVision .eeg format
        ...
        Note: this function is built on pybv
        """
        ...
        if idx_fold is not None:
            ...
            if self.resampled:
                operate_dataset = self.orig_cleaned_dataset
            else:
                operate_dataset = self.cleaned_dataset
            cleaned_data = operate_dataset.get_data()
            sfreq = operate_dataset.info['sfreq']
            ch_name = operate_dataset.info['ch_names']
            #events,events_id = mne.events_from_annotations(operate_dataset)
        # Save the dataset
        if overwrite or (p_output_curr / f_output).exists() is False:
            write_brainvision(data=cleaned_data, sfreq=sfreq, ch_names=ch_name,
                              fname_base=f_output, folder_out=str(p_output))
