...
class Session:
    def __init__(self, str_sub, vec_idx_run, str_arch, random_seed=1997, verbose=2, overwrite=False,
        ...
        self.p_model_weights = self.cfg.p_model_weights
    ...
    def prepare_training(self):
        ...
        if not self.cv_mode:
            ...
            if str(self.p_model_weights).strip() == '':
              self.session_model.compile_model()
            else: 
              try:
                self.session_model.load_model_weights(self.p_model_weights)
              except:
                raise Exception('Something wrong with p_model_weights')
              self.session_model.compile_model()
            ...
            for i in range(self.num_fold):
                ...
                if str(self.p_model_weigths.strip()) == '':
                   fold_model.compile_model()
                else:
                   try:
                      fold_model.load_model_weights(self.p_model_weights)
                   except:
                      raise Exception('Something wrong with p_model_weights')
                fold_model.compile_model()
    ...
    def save_databv(self):
        """
        Save the cleaned dataset in BrainVision files
        """
        # modified by jiayi 2021/6/24 for export BrainVision files.

        p_output = self.d_output / self.str_sub
        if not self.cv_mode:
            for i in range(len(self.vec_dataset)):
                curr_dataset = self.vec_dataset[i]
                f_output = self.output_file_naming_format.format(self.str_sub, self.vec_idx_run[i])

                curr_dataset.save_databv(p_output, f_output, self.overwrite)
        else:
            for idx_fold in range(self.num_fold):
                for idx_run in range(len(self.vec_dataset)):
                    curr_dataset = self.vec_dataset[idx_run]
                    f_output = self.cv_output_file_naming_format.format(self.str_sub,
                                                                        self.vec_idx_run[idx_run], idx_fold)

                    curr_dataset.save_databv(p_output, f_output, self.overwrite, idx_fold=idx_fold)
