...
%----------------------------------------------------------------------
beta  = swe_data_read(SwE.Vbeta, 'xyz', XYZ);
...
for j = 1:size(Co,1)
    for jj = j:size(Co,1)
        ...
        if any(j == ind) & any(jj == ind)
            ...
            Bcov = Bcov + weight * swe_data_read(SwE.Vcov_beta(it), 'xyz', XYZ);
        end
    end
end
CI    = spm_invTcdf(1-0.025,swe_data_read(SwE.xCon(Ic).Vedf, 'xyz', XYZ));
