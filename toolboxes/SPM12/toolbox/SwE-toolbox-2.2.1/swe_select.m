function varargout = swe_select(varargin)
  ...
  %
  % FORMAT [files,dirs] = swe_select('List',direc,filt)
  ...
  %
  % FORMAT [files,dirs] = swe_select('ExtList',direc,filt,frames)
  ...
  %
  % FORMAT [files,dirs] = swe_select('FPListRec',direc,filt)
  % FORMAT [files,dirs] = swe_select('ExtFPListRec',direc,filt,frames)
  % As above, but return files with full paths (i.e. prefixes 'direc' to each)
  % and search through sub directories recursively.
  %
  % FORMAT [dirs] = swe_select('List',direc,'dir',filt)
  % FORMAT [dirs] = swe_select('FPList',direc,'dir',filt)
  % FORMAT [dirs] = swe_select('FPListRec',direc,'dir',filt)
  ...
  %
  % FORMAT [t,ind] = swe_select('Filter',files,typ,filt,frames)
  ...
  %
  % FORMAT swe_select('PrevDirs',dir)
  % Add directory dir to list of previous directories.
  % FORMAT dirs = swe_select('PrevDirs')
  ...
  switch lower(cmd)
      case 'list'
          ...
          elseif numel(frames)~=1 || frames(1)~=1
              ...
              for i=1:numel(files)
                  try
                      n = swe_select_get_nbframes(fullfile(dr,files{i}));
                      d4 = (1:n)';
                  catch
                      d4 = 1;
                  end
  ...
  %==========================================================================
  % FUNCTION ofiles = swe_select_expand(ifiles)
  %==========================================================================
  function ofiles = swe_select_expand(ifiles)
  ...
  for i=1:numel(ifiles)
      ...
      if ~isempty(n)
          ...
          n = swe_select_get_nbframes(ifiles{i});
          vfiles = cell(n,1);
          for j=1:n
  ...
  %==========================================================================
  function n = swe_select_get_nbframes(file)
    file_ext = swe_get_file_extension(file);
    isCifti  = strcmpi(file_ext,'.dtseries.nii') ||  strcmpi(file_ext,'.dscalar.nii');
    if isCifti
      ciftiObject = swe_cifti(file);
      n = ciftiObject.dat.dim(5);
    else
      N   = nifti(file);
